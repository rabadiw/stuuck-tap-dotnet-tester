# Steeltoe AccountApp Accelerator
...
```
tanzu apps workload create stuuck-tap-dotnet-tester -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/stuuck-tap-dotnet-tester-source \
  --type web
```
...
```
tanzu apps workload get stuuck-tap-dotnet-tester
```
...
```
dotnet publish -o publish AccountApp.csproj
```

Push the app to Cloud Foundry:

```
cf push stuuck-tap-dotnet-tester -p ./publish --random-route
```
